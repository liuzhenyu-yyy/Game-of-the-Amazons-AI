	...
	for (int i = 0; i < GRIDSIZE; i++) {
		for (int j = 0; j < GRIDSIZE; j++) {
			if (gridInfo[i][j] == color) {
				flag = 0;
				for (int k = 0; k < 8; k++) {
					if (inMap(i + dx[k], j + dy[k]) && (gridInfo[i + dx[k]][j + dy[k]] == 0 || gridInfo[i + dx[k]][j + dy[k]] == color))
						flag = 1;
				}
				if (flag == 0) {
					cnt++;
					...
					for (int l = 0; l < 8; ++l) {
						for (int delta2 = 1; delta2 < GRIDSIZE; delta2++) {
							...
							if (ProcStep(i, j, xx, yy, xxx, yyy, currBotColor, true))
							{
								...
								if (ID <= 6) {			//前中后期比重
									score[posCount] = CalScore(0, 0, 0.5, 0.5, 0);
								}
								else if (ID <= 15) {
									score[posCount] = CalScore(0, 0, 0.5, 0.5, 0);
								}
...
int main()
{
	...
	Lock_Score(currBotColor, turnID);
	//cout<<ProcStep(4,6,7,3,3,7,currBotColor, false);
	return 0;
}
